...
export default function Auth() {
  ...
  // Simula o fluxo OAuth do Google. Para ativar de verdade, troque por
  // Google Identity Services (GSI) com o client_id do projeto.
  const loginGoogle = () => {
    if (gLoading || loading) return;
    setGLoading(true);
    setTimeout(() => { setGLoading(false); setDone(true); }, 1200);
  };
    ...
    <div className="auth">
      ...
      <main className="auth-main">
        <form className="auth-card" onSubmit={submit} noValidate>
          ...
          <button type="button" className="btn-google" onClick={loginGoogle} disabled={gLoading}>
            {gLoading
              ? <><span className="spinner"></span> Conectando ao Google…</>
